...
function buildDrafts(entries: TranslationMemoryEntry[]): Record<string, MemoryDraft> {
...
export function MemoryInlineTable({
  entries,
  busyKey,
  onSave,
  onDelete,
}: MemoryInlineTableProps) {
  const [drafts, setDrafts] = useState<Record<string, MemoryDraft>>(() => buildDrafts(entries))
  const [newDraft, setNewDraft] = useState<MemoryDraft>(EMPTY_DRAFT)
          ...
          <tr className="knowledge-inline-new-row">
            ...
            <td>
              <textarea
                ...
                placeholder="Add memory translation"
                onChange={(event) =>
            ...
            <td>
              <button
                ...
                disabled={busyKey === 'memory-save'}
                onClick={() => {
                  void onSave(newDraft).then(() => setNewDraft(EMPTY_DRAFT))
                }}
              >
                {busyKey === 'memory-save' ? 'Saving...' : 'Add'}
              </button>
            </td>
          </tr>
          {entries.map((entry) => {
              ...
              <tr key={entry.id}>
                ...
                <td>
                  <div className="knowledge-inline-actions">
                    ...
                    >
                      {busyKey === 'memory-save' ? 'Saving...' : 'Save'}
                    </button>
                    <button
                      type="button"
                      className="history-delete-button"
                      disabled={busyKey === `memory-delete:${entry.id}`}
                      onClick={() => {
                        void onDelete(entry.id)
                      }}
                    >
                      {busyKey === `memory-delete:${entry.id}` ? 'Deleting...' : 'Delete'}
                    </button>
                  </div>
                </td>
              </tr>
